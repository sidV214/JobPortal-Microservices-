...
/*
 * ===========================================================================================
 *                              NOTES — frontend/src/app/company/[id]/page.tsx
 * ===========================================================================================
 *
 * PURPOSE: Company detail page. Shows company profile, job listings, and recruiter management tools.
 * This is the LARGEST frontend page (~775 lines) because it handles both public view AND recruiter
 * management (job posting, editing, deleting, and application review) all in one page.
 *
 * PARAMS: [id] → company ID from URL
 *
 * PUBLIC VIEW (anyone):
 * - Company name, logo, description, website
 * - List of job cards posted by this company
 *
 * RECRUITER VIEW (if user owns the company):
 * - Create new job form (dialog)
 * - Edit existing jobs
 * - View applications for each job (sorted by subscription priority)
 * - Update application statuses (Hired/Rejected → triggers Kafka email)
 * - Delete company
 *
 * CONNECTIONS:
 * • Job Service /api/job/company/:id → company details + jobs
 * • Job Service /api/job/new → create job
 * • Job Service /api/job/:jobId → update job
 * • Job Service /api/job/application/:jobId → view applications
 * • Job Service /api/job/application/update/:id → update status
 */
