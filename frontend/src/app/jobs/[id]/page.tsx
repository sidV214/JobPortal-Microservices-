...
/*
 * ===========================================================================================
 *                              NOTES — frontend/src/app/jobs/[id]/page.tsx
 * ===========================================================================================
 *
 * PURPOSE: Individual job detail page. Shows full job description, company info, and apply button.
 *
 * PARAMS: [id] → job ID from URL (useParams)
 *
 * DATA FLOW:
 * 1. useEffect → GET job_service/api/job/:jobId → fetches full job data
 * 2. Displays all fields: title, description, salary, location, job_type, work_location, openings
 * 3. If recruiter owns the job → shows applications list with status update controls
 *
 * RECRUITER VIEW:
 * - Sees all applications for this job (sorted by subscribed priority)
 * - Can update application status (Submitted → Hired/Rejected)
 * - Updates trigger Kafka email to applicant
 *
 * JOBSEEKER VIEW:
 * - Sees job details + "Apply" button (if not already applied)
 *
 * CONNECTIONS:
 * • Job Service /api/job/:jobId → job details
 * • Job Service /api/job/application/:jobId → applications (recruiter only)
 * • Job Service /api/job/application/update/:id → status update
 */
